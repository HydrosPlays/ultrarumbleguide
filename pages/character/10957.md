...
### Beta Skill (Loop Time Ground)
The time the skill is looped when used on the ground after the damaging part of the kick is gone. High values extend the kick's travel time while lacking the damage hitbox from the kick. The values are:<br>
- 0.1 seconds (Level 1-3)
- 0.2 seconds (Level 4-8)
- 0.3 seconds (Level 9)

### Beta Skill (Loop Kick Speed Ground)
This is the kick's speed when used on the ground. High values make the kick super fast, allowing Deku to travel and damage far away enemies. The values are:<br>
- 3000 UU/s (All Levels)

### Beta Skill (Kick Anim Rate Ground)
Deku's kick animation is split into 3 parts: The little turnaround he does before actually doing the kick-dash, the damaging kick dash, and the looped non-damaging kick dash. This parameter modifies the speed of the little turnaround when used on the ground, meaning that really small values won't change the kick itself, but will make the little start animation before take forever to perform. The values are:<br>
- 5 (Level 1-3)
- 7 (Level 4-9)

### Beta Skill (Loop Time Air)
The time the skill is looped when used mid-air after the damaging part of the kick is gone. High values extend the kick's travel time while lacking the damage hitbox from the kick. The values are:<br>
- 0.1 seconds (Level 1-3)
- 0.2 seconds (Level 4-8)
- 0.3 seconds (Level 9)

### Beta Skill (Loop Kick Speed Air)
This is the kick's speed when used mid-air. High values make the kick super fast, allowing Deku to travel and damage far away enemies. The values are:<br>
- 3000 UU/s (All Levels)

### Beta Skill (Kick Anim Rate Air)
Deku's kick animation is split into 3 parts: The little turnaround he does before actually doing the kick-dash, the damaging kick dash, and the looped non-damaging kick dash. This parameter modifies the speed of the little turnaround when used mid-air, meaning that really small values won't change the kick itself, but will make the little start animation before take forever to perform. The values are:<br>
- 5 (Level 1-3)
- 7 (Level 4-9)

### Beta Skill (Kick Super Armor Ground)
Determines whether this skill has superarmor when used on the ground. It's currently set to True.

### Beta Skill (Kick Super Armor Air)
Determines whether this skill has superarmor when used mid-air. It's currently set to True.
...
### Beta Skill (Move Time Limit)
This parameter decides how long Deku's initial animation pre-jump lasts. High values make Deku freeze in a "attempting to jump" animation while slowly gliding forward until the time stated in the parameter has passed.
- 0.25 (All Levels)

### Beta Skill (Fall Time)
This parameter controls for how long can Deku fall while using beta. This value is the reason why when you use beta from very high places he stomps the air after a while. Very low values make him do the stomp sooner than expected even if Deku is mid-air.
- 6 seconds (All Levels)

### Beta Skill (Fall Init Speed)
This parameter controls how fast Deku falls during his beta initially, which ramps up until it reaches the value in his "Fall Last Speed" parameter. Positive values make the beginning of the fall turn into a rise, before going downwards again. The values are:<br>
- -3000 UU/s (All Levels)

### Beta Skill (Fall Last Speed)
This parameter controls how fast Deku falls during his beta after a bit of falling. Positive values make the fall turn into a rise. The values are:<br>
- -5000 UU/s (All Levels)

### Beta Skill (Fall Acceleration Span)
This parameter controls how fast Fall Init Speed transitions into Fall Last Speed. The values are:<br>
- 2 seconds (All Levels)

### Beta Skill (Slide Speed)
This parameter controls the speed at which Deku moves when inputting a direction while falling mid-beta. High values will allow Deku to move superfast in any direction that isn't up or down while falling mid-beta. The values are:
- 350 UU/s (All Levels)

### Beta Skill (Slide Span)
This parameter controls the speed at which Deku transitions into his Slide Speed mid-beta fall. The values are:
- 0.1 seconds (All Levels)

### Beta Skill (Low Height)
This parameter tells the game when has Deku fallen from high enough to turn his base height kick into a Low Height kick. When set to 1, Deku always does increased damage. The values are:
- 1500 UU (All Levels)

### Beta Skill (Middle Height)
This parameter tells the game when has Deku fallen from high enough to turn his base height kick into a Middle Height kick. The value is the same as his Low Height, so this issue might be related to how they "removed" one of Strike Deku's beta fall levels. The valies therefore are:
- 1500 UU (All Levels)
